;(function($){
    ...
    function editorPaste(_this){
        console.log('-------------editorPaste------------');
       // _this._conWin.document.body.innerHTML = clearformat(_this._conWin.document.body.innerHTML);
    }
    ...
    // function clearformat(_html){
    //     //删除属性
    //     var replaceTagArr = ['div'];
    //     var removeAttr =  /\w*\s*=\s*(((['"])([^'"]*\3))|[^\s]*)/mg
    //     //过滤属性
    //       ,ignoreAttr = /src/ 
    //       //需替换的标签
          
    //       ,replaceTag = new RegExp('<\/(' + replaceTagArr.join('|') + ')>|<(' + replaceTagArr.join('|') + ')[^>]*>', 'mg')
    //     //删除标签
          
    //     ,removeTag = /<\/([^>]+)>|<([^>]+)>/mg
    //     //过滤标签
    //       ,ignoreTag = /<\/?(p|img|table|tr|td)/;
          
          
    //         //匹配标签
    //       _html = _html.replace(removeTag, function(hit, end, start){
    //         //查找可忽略的标签
    //           if(ignoreTag.test(hit)){
    //               if(start){
    //                   hit = hit.replace(removeAttr, function(hit){
    //                       return ignoreAttr.test(hit) ? hit : '';
    //                   });
    //               }
    //           }else{
    //               if(replaceTag.test(hit)){
    //                   hit = hit.replace(replaceTag, function(hit, end, start){
    ...
    var commandMap = {
            source : function(){
                ...
                if(this.sourceIsDisplay){
                    ...
                    console.log(this.getDom().childNodes[2].childNodes[0].value);
                    console.log($.HtmlHelper.HTMLtoXML(this.getDom().childNodes[2].childNodes[0].value));
                    // this._conWin.document.body.innerHTML = $.HtmlHelper.HTMLtoXML(this.getDom().childNodes[2].childNodes[0].value);
                    ...
                    console.log(this._conWin.document.body.innerHTML);
                    console.log($.HtmlHelper.HTMLtoXML(this._conWin.document.body.innerHTML, true));
            ...
            ,clearformat : function(){
                $.logout(clearformat(this.getValue()));
                this.setValue(clearformat(this.getValue()));
            }
    ...
    Editor.prototype = $.ObjectHelper.extend({
        ...
        ,_iframeReady : function(){
           ...
           //this._conWin.focus();
            ...
            var ttt = this;
            this._conWin.document.body.addEventListener('paste',editorPaste, false);  
        ...
        ,setValue : function(_val){
            ...
            window.setTimeout(function(){
                self._conWin.document.body.innerHTML = clearformat(_val);//$.HtmlHelper.HTMLtoXML(_val);
            }, 1000);
...
(function(P){
    ...
    _this = P.html = {
        ...
        /** 默认正则表达式 */
        regstrs : {
            removeAttr : '\\w*=[\'\"]*[\\w\\d:#\.\\-;\(\)\/]+[\'\"]*'//删除属性
            ,ignoreAttr : 'href|src'//过滤属性
            ,replaceTag : '<(\\w*)\\s*>|<\/(\\w*)>'//删除标签
            ,ignoreTag : '<\/?(img|table)'//过滤标签
        },
        /** 初始化RegExp对象 */
        regs : {
            removeAttr : null,
        ...
        /** 自定义过滤配置
            {
                ignoreAttr : []//过滤属性,字符串数组
                ,ignoreTag : []//过滤标签,字符串数组
            }
         */
        config : function(options){
            if (options && options.ignoreAttr) {
                _this.regstrs.ignoreAttr = options.ignoreAttr;
            }
            if (options && options.ignoreTag) {
                _this.regstrs.ignoreTag = options.ignoreTag;
            }
        },
        /** 加载配置 */
        loadConfig : function(){
            var config = _this.defaultConfig;
            _this.regstrs.ignoreAttr = config.ignoreAttrs.join('|');
            _this.regstrs.ignoreTag = '<\/?('+ config.ignoreTags.join('|') +')';

            _this.regs.removeAttr = new RegExp(_this.regstrs.removeAttr, 'g');
            _this.regs.ignoreAttr = new RegExp(_this.regstrs.ignoreAttr, '');
            _this.regs.replaceTag = new RegExp(_this.regstrs.replaceTag, 'g');
            _this.regs.ignoreTag  = new RegExp(_this.regstrs.ignoreTag, '');
        },
        _cleanNode : function(node){
            if (!node) {
             ...
             console.log(node);
            if (_this._isEmptyNode(node)){
                node.remove();
            }else{
                if (node.nodeType == 3 || node.localName == null) {
                    return;
                }
                if (node.localName && _this.defaultConfig.ignoreTags.contains(node.localName)) {
        ...
        function Parser() {
            ...
            this.get_token = function() {
                ...
                if (this.last_token === 'TK_TAG_SCRIPT') {
                    ...
                    //token = <a href="https://www.baidu.com/s?wd=js&tn=44039180_cpr&fenlei=mv6quAkxTZn0IZRqIHckPjm4nH00T1YLuWP-PHRdrHTYrHR1mvRY0ZwV5Hcvrjm3rH6sPfKWUMw85HfYnjn4nH6sgvPsT6K1TL0qnfK1TL0z5HD0IgF_5y9YIZ0lQzqlpA-bmyt8mh7GuZR8mvqVQL7dugPYpyq8Q1csPHcLnHRdnjDkn10zPHc4PjT" target="_blank" class="baidu-highlight">js</a>_beautify(temp_token, this.indent_size, this.indent_character, this.indent_level);
